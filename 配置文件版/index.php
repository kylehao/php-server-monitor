 ...
 * 版本：2.0
 * 作者：你的昵称（可修改）
 * 说明：本文件为服务器状态展示页，配置从 config.json 读取
 * ==========================================
 */
...
// 配置文件路径
$CONFIG_FILE = __DIR__ . "/data/config_xxxxxxxxxxxxxxxxxxxx.json";
...
foreach ($servers as $server) {
    ...
    $group_icon = $server['group_icon'] ?? 'fa-folder';
    $traffic_limit = $server['traffic_limit'] ?? 1000;
    $custom_tags = $server['custom_tags'] ?? [];
    $expire_date = $server['expire_date'] ?? '';
    $other_tags = $server['other_tags'] ?? [];
    $flag = $server['flag'] ?? '';
    
    if (!empty($token_md5) && !empty($name)) {
        $TOKENS[$token_md5] = $name;
    }
    
    if (!empty($group) && !empty($name)) {
        if (!isset($SERVER_GROUPS[$group])) {
            $SERVER_GROUPS[$group] = [];
        }
        $SERVER_GROUPS[$group][] = $name;
        $GROUP_ICONS[$group] = $group_icon;
    }
    
    if (!empty($name)) {
        $TRAFFIC_LIMIT[$name] = $traffic_limit;
        $CUSTOM_TAGS[$name] = $custom_tags;
        $EXPIRE_DATES[$name] = $expire_date;
        $OTHER_TAGS[$name] = $other_tags;
        if (!empty($flag)) {
            $FLAG_MAP[$name] = $flag;
        }
    }
